...
class Node{
    ...
    string name;
    int age;
    //A node pointer for next node
    Node*next; 
    //A node pointer for previous node
    Node*prev; 
    //calling constructor
    Node(string name,int age){
        ...
        next=NULL; 
        //initialising prev as NULL
        prev=NULL; 
    }
};

//FUNCTION TO INSERT DATA AT THE END OF LINKED LIST
void append(Node* &head,string name,int age){
    //Creating a node pointer and storing address of head in temp
    Node*temp=head;
    //creating a new node to store name and age in it
    Node*new_node=new Node(name,age);
    //Inserting the created node in empty list
    if(temp==NULL){
        head=new_node;
    }
    //Inserting the created node in non empty list
    else{
...
//FUNCTION TO DIPLAY DATA FROM THE START OF LINKED LIST
...
//FUNCTION TO DISPLAY DATA FROM THE END OF LINKED LIST
...
//FUNCTION TO DELETE ANY INDEX EXCLUDING LAST ELEMENT 
...
int main(){
    ...
    cout<<endl;
    cout<<"Doubly Linked list with family members is:"<<endl;
    cout<<endl;
